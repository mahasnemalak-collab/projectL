...
<div class="flex space-x-5 text-center flex items-center justify-center min-h-screen bg-[url('{{asset('img/hand.jpg')}}')] bg-cover bg-center ">
        
    <div class=" flex items-center justify-center w-1/2 p-6 bg-white shadow-lg rounded-lg text-center">
        <audio controls class="mx-auto">
            <source src="{{asset('audio/kids.ogg')}}" type="audio/mpeg">
            Your browser does not support the audio element.
        </audio>

        <form action="/test/Understanding-speaking/understanding_kids/post" method="POST" class="mt-4">
          @csrf
            <label for="listeningInput" class="block text-lg font-semibold">What did you hear?</label>
            <select name="option1" id="">
                 <option value="1">A, U, O</option>
                 <option value="2">A, E, Y</option>
                 <option value="3">A, B, P</option>
                 <option value="4">Y, E, A</option>
            </select>
